...
Corresponding author: Dr. Carina A. Hahn, <carina.hahn@nist.gov> 
...
```{r load packages and source functions, include=FALSE}
library(tidyverse)
library(knitr)
library(kableExtra)
library(ez)
...
## Obtaining
De-identified data can be obtained via data transfer agreement with the National Institute of Standards and Technology (NIST). Contact Dr. Carina Hahn at <carina.hahn@nist.gov> to obtain a data transfer agreement.
...
with(analysis.data,suppressWarnings(ks.test(Scores[Group=="1to1Face" & mateBinary == "1"],Scores[Group=="Super" & mateBinary == "1"])))
```

**Examiner vs. Super: Different-identity**

```{r kstest exam vs super nonmatch}
with(analysis.data,suppressWarnings(ks.test(Scores[Group=="1to1Face" & mateBinary == "0"],Scores[Group=="Super" & mateBinary == "0"])))
```

**Examiner: Same- vs. Different-identity**

```{r kstest examiner m v nm}
subset1<-analysis.data$Scores[analysis.data$Group=="1to1Face" & analysis.data$mateBinary=="1"]
subset2<-analysis.data$Scores[analysis.data$Group=="1to1Face" & analysis.data$mateBinary=="0"]*-1 #reversed for mirrored judgments

ks.test(subset1,subset2)
rm(subset1,subset2)
```

**Super-recognizer: Same- vs. Different-identity**
```{r kstest super m v nm}
subset1<-analysis.data$Scores[analysis.data$Group=="Super" & analysis.data$mateBinary=="1"]
subset2<-analysis.data$Scores[analysis.data$Group=="Super" & analysis.data$mateBinary=="0"]*-1 #reversed for mirrored judgments
...
```{r high low confidence descriptive statistics}
analysis.data %>%
...
#### High confidence
```{r high conf all trial mann whit}
analysis.data %>%
  group_by(SubID,Group,Scores) %>%
  tally() %>%
  spread(Scores,n) %>%
...
```{r low conf all trial mann whit}
analysis.data %>%
  group_by(SubID,Group,Scores) %>%
  tally() %>%
  spread(Scores,n) %>%
...
plot1.temp<-analysis.data %>%
  ...
  ggplot(.,aes(y=Scores,x=Difficulty,color=fct_rev(mateBinary)))+
